...
# cursor.execute('SELECT count(product_name) FROM globus;') #1
# pp(cursor.fetchall())

# cursor.execute('SELECT product_name FROM narodnii GROUP BY product_id HAVING day_to_expire < 2;') #2
# pp(cursor.fetchall())

cursor.execute('SELECT count(*) FROM globus WHERE;')
